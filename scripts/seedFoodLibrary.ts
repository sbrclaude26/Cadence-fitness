...
async function main() {
  ...
  // Load curated + USDA bulk. Order matters: USDA first, then curated, so the
  // hand-authored entries win on slug collisions (curated names like
  // "Chicken breast, raw" override the verbose USDA description for the same
  // food). usda-foods.json is generated by scripts/buildUsdaJson.ts.
  ...
  try {
    ...
    console.warn("usda-foods.json not found — run `npx tsx scripts/buildUsdaJson.ts` first to include the USDA bulk set.");
  }

  const bySlug = new Map<string, CuratedFood>();
  for (const f of usda) bySlug.set(f.slug, f);
  for (const f of curated) bySlug.set(f.slug, f); // curated wins
