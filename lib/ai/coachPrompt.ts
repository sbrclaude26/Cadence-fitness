...
- WORKOUT SESSION INTELLIGENCE: You will receive recent Watch-recorded workout sessions (cardio, walks, strength). Use this data to:
  ...
  • COLLECTIVE METRICS, NOT PER-EXERCISE — CRITICAL: Each workout_sessions row carries an 'associated_exercises' array listing the logged strength exercises the athlete has tied to that Apple Watch session. The session's metrics (duration_min, calories, avg_hr, max_hr, distance_km) are the **TOTAL for the whole watch session, shared across every exercise in associated_exercises** — they are NOT incurred by each exercise individually. Example: a 50-min Traditional Strength Training session with 670 cal and associated_exercises = [Bench, OHP, Plank] means Bench+OHP+Plank combined produced 670 cal — NOT 670 each. Never attribute session-level metrics per-exercise; never multiply or sum them across the exercises in the list. When you reference these metrics in whatChangedWorkouts, frame them at the session level ("across Bench, OHP, and Plank you held a max HR of 156 and burned ~670 cal in 50 min"). The 'exerciseHistory' entries carry 'workout_session_id' so you can cross-reference which logged exercise belongs to which Apple Watch session.
  • OPTIONAL APPLE WATCH DATA: associated_exercises may be empty (a watch session the athlete logged but didn't tie to any strength work — e.g. a standalone Indoor Walk) and exerciseHistory entries may have workout_session_id = null (logged exercise with no Apple Watch session attached — common; the athlete may not have worn the watch, or simply didn't link it). Reason gracefully in both cases — never assume watch coverage. When watch data is absent, lean on logged sets/reps/load/RPE alone and say so briefly in whatChangedWorkouts.
...
export function buildUserContext(ctx: {
  ...
  recentWorkoutSessions: Array<{ id: string; date: string; type: string; name: string | null; library_slug: string | null; description: string | null; duration_min: number | null; distance_km: number | null; calories: number | null; avg_hr: number | null; max_hr: number | null; avg_speed_mph?: number | null; avg_incline_pct?: number | null; planned_exercise_name?: string | null; position_in_session?: number | null; notes?: string | null; associated_exercises: Array<{ exercise_name: string; date: string; position_in_session: number | null }> }>;
