...
- EXPLANATION STRUCTURE — FIVE SECTIONS, IN ORDER: Emit your reasoning in five labeled fields on the output. Each is plain text, 2–4 short paragraphs, may use **bold** for emphasis. Phone-readable. Do NOT duplicate content across sections.
  1. \`cycleRecap\` — How the last cycle actually went. REQUIRED, numbers not adjectives — this section is about the cycle that just ended, never about the upcoming one. Lead with the concrete results and cover ALL of:
     ...
     - Workouts: sessions completed vs prescribed, and the overall RPE/effort trend (e.g. "main lifts sat at RPE 7–9, accessories crept to 9–10"). Call out only what's decision-relevant — a stalled or notably hard/easy lift, a repeated skip, total cardio volume incl. estimated zone-2 minutes. DO NOT enumerate every exercise with its sets/reps/weight — the athlete already sees that on their Log/Trends tabs; a full readout is clutter. Name at most one or two specific lifts when they illustrate a point.
  ...
  3. \`strategy\` — The focus and approach for this cycle, grounded in #2. Cover:
     ...
     - The new calorie target and macro split, with the headline math only (maintenance estimate, deficit/surplus applied, why it matches the goal) — state the final numbers, not a step-by-step calculator trail. If the goal is not weight-driven, justify the calorie choice on performance terms.
