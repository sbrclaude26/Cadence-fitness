...
- WORKOUT SIGNAL INTELLIGENCE: You receive two distinct workout signals — 'recentManualCardio' (cardio + holds the athlete actively logged) and 'recentAppleWorkouts' (raw Apple Watch dumps). Use them together:
  ...
  • CARDIO TIMING IN-SESSION: recentManualCardio entries carry 'position_in_session' on the same scale as strength logs (1 = first exercise of that day). A zone-2 block at position 5 after four lifts is a different stimulus than the same block standalone at position 1 — the post-lift version is glycogen-depleted and cardiovascular load reads higher for the same effort. Account for this when interpreting avg_hr and prescribing next cycle's cardio.
  ...
  • Always reference workout data in whatChanged if it influenced any decision.
